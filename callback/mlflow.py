# -*- coding: utf-8 -*-
# @Time    : 2021/11/26 5:06 下午
# @Author  : JianingWang
# @File    : MlflowCallback
import os
import logging
import numpy as np
from tools.runner_utils.retrying import retry
...
class MLflowCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            experiment = self._ml_flow.get_experiment_by_name(
                self.experiment_name)
            if experiment:
                experiment_id = experiment.experiment_id
            else:
                if self.mlflow_location:
                    artifact_location = os.path.join(self.mlflow_location,
                                                     'artifact')
                    if not os.path.exists(artifact_location):
                        os.makedirs(artifact_location)
                    experiment_id = self._ml_flow.create_experiment(
                        self.experiment_name,
            ...
            self._ml_flow.start_run(experiment_id=experiment_id,
                                    run_name=self.run_name)

            combined_dict = OrderedDict()
            for arg, ori in [[self.data_args, DataTrainingArguments],
                             [self.model_args, ModelArguments],
                             [self.training_args, TrainingArguments]]:
                for k, v in arg.to_dict().items():
                    # 只有和参数默认值不一致的时候才保留参数
                    if (hasattr(ori, k) and getattr(ori, k) == v) or k in [
                            'tracking_uri', 'log_level', 'log_level_replica',
            ...
            for i in range(0, len(combined_dict_items),
                           self._MAX_PARAMS_TAGS_PER_BATCH):
                self._log_params(
                    dict(combined_dict_items[i:i +
                                             self._MAX_PARAMS_TAGS_PER_BATCH]))
    ...
    @retry(stop_max_attempt_number=5,
           wait_fixed=1000,
           after_attempts=lambda x: print(f'尝试log metrics失败, retry第{x}次'),
           skip_raise=True)
    ...
    @retry(stop_max_attempt_number=5,
           wait_fixed=1000,
           after_attempts=lambda x: print(f'尝试log params失败, retry第{x}次'),
           skip_raise=True)
    ...
    @retry(stop_max_attempt_number=5,
           wait_fixed=1000,
           after_attempts=lambda x: print(f'尝试end run失败, retry第{x}次'),
           skip_raise=True)
    def _end_run(self):
