...
class MLflowCallback(TrainerCallback):
    ...
    @retry(stop_max_attempt_number=5, wait_fixed=1000, after_attempts=lambda x: print(f'尝试log metrics失败, retry第{x}次'), skip_raise=True)
    def _log_metrics(self, metrics, step):
        self._ml_flow.log_metrics(metrics, step)

    @retry(stop_max_attempt_number=5, wait_fixed=1000, after_attempts=lambda x: print(f'尝试log params失败, retry第{x}次'), skip_raise=True)
    def _log_params(self, params):
        self._ml_flow.log_params(params)

    @retry(stop_max_attempt_number=5, wait_fixed=1000, after_attempts=lambda x: print(f'尝试end run失败, retry第{x}次'), skip_raise=True)
    def _end_run(self):
