...
class ModelArguments:
    ...
    early_stopping_patience: int = field(
        default=None,
        metadata={
            'help': 'early_stopping_patience for early stopping callback'
        })

    freeze_epochs: Optional[float] = field(
        default=None, metadata={'help': '对前n个epoch进行冻层操作, float类型如 0.5'})
    freeze_keyword: Optional[str] = field(
        default='encoder', metadata={'help': '包含keyword的layer将会被冻层'})

    ema: bool = field(default=False, metadata={'help': '是否使用ema'})

    ema_decay: float = field(default=0.999, metadata={'help': 'ema系数'})
    ...
    use_prompt_for_cls: bool = field(
        ...
        metadata={
            'help':
            'Whether to use prompt-based learning settings. If true, that means use pre-trained task with specific'
    ...
    use_freezing: bool = field(
        ...
        metadata={
            'help':
            'Whether to use parameter-efficient settings. If true, that means freezing the parameters of the backbone, and only'
    ...
    pre_seq_len: int = field(default=4,
    ...
    prefix_hidden_size: int = field(
        ...
        metadata={
            'help':
            'The hidden size of the MLP projection head in Prefix Encoder if prefix projection is used'
        })
...
class DataTrainingArguments:
    ...
    task_type: Optional[str] = field(
        ...
        metadata={'help': '任务类型：classification, mlm'})
    data_dir: Optional[str] = field(default=None, metadata={'help': '数据路径'})
    ...
    mlflow_location: Optional[str] = field(
        default=None,
        metadata={'help': 'The location of mlflow tracking and artifact'})
    ...
    test_file: Optional[str] = field(
        default=None, metadata={'help': 'An optional input test data file.'})
    ...
    max_eval_seq_length: Optional[int] = field(
        ...
        metadata={
            'help':
            'The maximum total input sequence length after tokenization for eval and test set. Sequences longer '
            'than this will be truncated.'
        },
    )
    ...
    cross_validation_num: Optional[int] = field(
        default=None, metadata={'help': 'cross_validation_num'})
    ...
    post_tokenizer: Optional[bool] = field(default=False,
                                           metadata={'help': 'post_tokenizer'})
    user_defined: Optional[str] = field(
        default='',
        metadata={
            'help':
            "User defined by you-self, split by '\space' e.g. 'name=xxx year=2000'"
        })
...
class TrainingArguments(TransformersTrainingArguments):
    ...
    do_predict_during_train: bool = field(default=False)

    pre_train_from_scratch: bool = field(default=False,
