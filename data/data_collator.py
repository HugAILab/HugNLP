...
# @dataclass
# class DataCollatorForLMWithoutNumber(DataCollatorForLanguageModeling):
#     """
#     在mlm MASK的时候不MASK数值
#     """
...
#         self.exclude_tokens = set(self.numerical_tokens + self.tokenizer.all_special_ids)
...
#         special_tokens_mask = sum(labels == i for i in self.exclude_tokens).bool()
...
class DataCollatorForPretrainWithKG(DataCollatorMixin):
    ...
    def torch_call(
        ...
        from tools.ner import position_2_bio
        for example in examples:
            ...
            if 'entity_position' in example:
                example['ner_labels'] = [
                    position_2_bio(p, seq_len)
                    for p in example['entity_position']
                ]
