...
all_processors = {
    task_name: task_processor
    for processor in all_processors
    for task_name, task_processor in processor.items()
}
...
class InContextLearningCollatorForGPT:
    ...
    def __call__(self, features):
        ...
        for i in range(0, len(batch['input_ids']), option_size):
            new_batch['input_ids'].append(batch['input_ids'][i:i +
                                                             option_size])
            new_batch['token_type_ids'].append(
                batch['token_type_ids'][i:i + option_size])
            new_batch['attention_mask'].append(
                batch['attention_mask'][i:i + option_size])
        ...
        batch['options'] = torch.Tensor(
            [list(range(len(f['options']))) for f in features]).long()
...
class InContextLearningProcessor(CLSProcessor):
    def __init__(self,
        ...
        self.task_format = param[
            'task_format']  # classification / qa / generation / probing
        ...
        self.options = [
            v[0] if type(v) == list else v for k, v in self.verbalizers
        ]  # label word list
        self.prompt_prefix = param[
            'prompt_prefix'] if 'prompt_prefix' in param.keys(
            ) else ''  # prefix template. e.g. Asnwer next question.
        self.q_prefix = param['q_prefix'] if 'q_prefix' in param.keys(
        ) else 'Input: '  # question predix template. e.g. Input:
        self.a_prefix = param['a_prefix'] if 'a_prefix' in param.keys(
        ) else 'Output: '  # answer prefix template. e.g. Output:
        # 确保dev和test使用的是相同的in-context example，因此在创建processor的同时，采样好对应的in-context example
        self.incontext_examples = self.InContextSampling(
            self.get_examples('train'))
    ...
    def get_data_collator(self):
        ...
        return InContextLearningCollatorForGPT(
            self.tokenizer,
            pad_to_multiple_of=8 if pad_to_multiple_of_8 else None,
            pad_to_max_length=self.data_args.pad_to_max_length)

    def InContextSampling(self,
                          examples: List[InputExample]) -> List[InputExample]:
        # used for sampling in-context examples
        random.seed = self.data_args.seed
        random.shuffle(examples)
        incontext_examples = examples[:self.
                                      num_incontext_example] if self.num_incontext_example > len(
                                          examples) else examples
        return incontext_examples

    def construct_prompt(self, incontext_examples: List[InputExample],
                         eval_example: InputExample):
        ...
        for incontext_example in incontext_examples:
            s = incontext_example['text_a'] + (
                ' ' + incontext_example['text_b']
            ) if incontext_example['text_b'] is not None else ''
            l = incontext_example['label']
            if self.task_format == 'classification':
                # 需要将类别标签转换为词
                label_word = self.verbalizers[l][0] if isinstance(
                    self.verbalizers[l], list) else self.verbalizers[l]
            else:
                label_word = l
        ...
        eval_s = eval_example['text_a'] + (
            ' ' + eval_example['text_b']
        ) if eval_example['text_b'] is not None else ''
        label = eval_example['label']
        prompt += self.q_prefix
        prompt += eval_s + '\n'
        assert self.a_prefix[-1] == ' '
        prompt += self.a_prefix[:
                                -1]  # GPT models do not want a trailing space, so we cut off -1
        return prompt, label
    ...
    def get_examples(self, set_type):
        ...
        if set_type == 'train':
            ...
            # 先获取所有的训练集
            training_examples = self.processor.get_train_examples(
                self.data_dir)
            ...
            if set_type == 'dev':
                examples: List[InputExample] = self.processor.get_dev_examples(
                    self.data_dir)
            elif set_type == 'test':
                examples: List[
                    InputExample] = self.processor.get_test_examples(
                        self.data_dir)
            # 为每个dev/test构建prompt
            eval_examples = list()
            for example in examples:
                prompt, label = self.construct_prompt(incontext_examples,
                                                      example)
            ...
            if self.task_format == 'classification':
                for option in self.options:
                    examples.append({
                        'id': id_,  # 测试样本编号
                        'prompt':
                        prompt.strip() + ' ' + option,  # 每个候选label对应一个prompt
