...
class DefaultSequenceClassificationProcessor(CLSProcessor):
    def __init__(self, data_args, training_args, model_args, tokenizer=None, post_tokenizer=False, keep_raw_data=True):
        ...
        self.train_file = os.path.join(data_args.data_dir, 'train.json') # each line: {"sentence1": xx, "sentence2": xx, "label": xx}
        self.dev_file = os.path.join(data_args.data_dir, 'dev.json')
        self.test_file = os.path.join(data_args.data_dir, 'test.json')
        self.label_file = os.path.join(data_args.data_dir, 'label_names.txt') # each line is one label name
        ...
        # 如果用户没有输入label name，检查本地是否有label_names.json文件
            ...
            raise FileNotFoundError("You must define the 'label_names' in user-define parameters or"
    ...
    def get_data_collator(self):
        ...
        return DataCollatorForDefaultSequenceClassification(self.tokenizer, max_length=self.data_args.max_seq_length, pad_to_multiple_of=8 if pad_to_multiple_of_8 else None, pad_to_max_length=self.data_args.pad_to_max_length)
