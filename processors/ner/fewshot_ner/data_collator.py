...
class DataCollatorForTokenProto:
    ...
    def __call__(self, features):
        ...
        for feature_id, feature in enumerate(features):  # 遍历每一个episode
            ...
            batch_support['sentence_num'].extend(
                feature['support_sentence_num'])
        ...
        batch_support = {
            keys: torch.tensor(values)
            for keys, values in batch_support.items()
        }
        batch_query = {
            keys: torch.tensor(values)
            for keys, values in batch_query.items()
        }
...
def generate_global_pointer_labels(data_set: dict, max_length):
    ...
    labeled_spans, labeled_types, offset_mappings = data_set[
        'labeled_spans'], data_set['labeled_types'], data_set['offset_mapping']
    new_labeled_spans, new_labeled_types = list(), list()
    labels = torch.zeros(
        len(labeled_spans), 1, max_length,
        max_length)  # 阅读理解任务entity种类为1 [bz, 1, max_len, max_len]
...
class DataCollatorForSpanProto:
    ...
    def __call__(self, features):
        ...
        for feature_id, feature in enumerate(features):  # 遍历每一个episode
            ...
            support_input, query_input = feature['support_input'], feature[
                'query_input']
            ...
            support_batch['attention_mask'].extend(
                support_input['attention_mask'])
            support_batch['token_type_ids'].extend(
                support_input['token_type_ids'])
            support_batch['offset_mapping'].extend(
                support_input['offset_mapping'])
            ...
            support_batch['labeled_spans'].extend(
                feature['support_labeled_spans'])
            support_batch['labeled_types'].extend(
                feature['support_labeled_types'])
            support_batch['sentence_num'].append(
                feature['support_sentence_num'])
        ...
        support_labels, support_new_labeled_spans, support_new_labeled_types = generate_global_pointer_labels(
            support_batch, self.max_length)
        query_labels, query_new_labeled_spans, query_new_labeled_types = generate_global_pointer_labels(
            query_batch, self.max_length)
        ...
        if support_batch['labels'].max() > 0:
            support_batch['short_labels'] = torch.ones(
                len(support_batch['labeled_spans']))
        else:
            support_batch['short_labels'] = torch.zeros(
                len(support_batch['labeled_spans']))

        query_batch['labels'] = query_labels
        if query_batch['labels'].max() > 0:
            query_batch['short_labels'] = torch.ones(
                len(query_batch['labeled_spans']))
        else:
            query_batch['short_labels'] = torch.zeros(
                len(query_batch['labeled_spans']))

        # convert to torch
        # id_batch = torch.Tensor(id_batch).long()
        support_batch['input_ids'] = torch.Tensor(
            support_batch['input_ids']).long()
        support_batch['attention_mask'] = torch.Tensor(
            support_batch['attention_mask']).long()
        support_batch['token_type_ids'] = torch.Tensor(
            support_batch['token_type_ids']).long()
        query_batch['input_ids'] = torch.Tensor(
            query_batch['input_ids']).long()
        query_batch['attention_mask'] = torch.Tensor(
            query_batch['attention_mask']).long()
        query_batch['token_type_ids'] = torch.Tensor(
            query_batch['token_type_ids']).long()
