...
class ChidMLMDataCollator:
    ...
    def __call__(self, features):
        labels = [feature.pop('label')
                  for feature in features] if 'label' in features[0] else None
        ...
        batch['input_ids'] = [
            self.cut_text(input_id) for input_id in batch['input_ids']
        ]
        batch['token_type_ids'] = [[0] * len(input_id)
                                   for input_id in batch['input_ids']]
        batch['attention_mask'] = [[1] * len(input_id)
                                   for input_id in batch['input_ids']]
...
class ChidMLMProcessor(CLSProcessor):
    ...
    def set_tokenizer(self, tokenizer):
        ...
        self.tag_token_id = self.tokenizer.convert_tokens_to_ids(
            self.tag_token)
    ...
    def compute_metrics(self, p: EvalPrediction):
        ...
        return {'eval_acc': round(acc, 4), 'eval_score': round(acc, 4)}
    ...
    def save_result(self, logits, labels):
        ...
        outfile = os.path.join(self.data_args.data_dir,
                               'c3_train_mlm_top20.json')
