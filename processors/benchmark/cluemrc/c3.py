...
class C3Processor(CLSProcessor):
    ...
    def get_examples(self, set_type):
        if set_type == 'train':
            examples = self._create_examples(
                self._read_json(self.mtrain_file) +
                self._read_json(self.dtrain_file) * 2, 'train')
            self.train_examples = examples
            if self.pseudo_train:
                predict_file = os.path.join(self.data_args.data_dir,
                                            'c3_predict.json')
                pse1 = self._create_examples(
                    self._read_json(self.mdev_file) * 2 +
                    self._read_json(self.ddev_file) * 2, 'train')
                pse2 = self._create_examples(self._read_json(predict_file),
                                             'train',
                                             pseudo=True)
                examples = examples + pse1 + pse2
        elif set_type == 'dev':
            examples = self._create_examples(
                self._read_json(self.mdev_file) +
                self._read_json(self.ddev_file), 'dev')
                ...
                if len(choices) > 4:
                    ...
                    for choice in choices[-4:]:
                        if choice[0] not in num_map:
                            ...
                            continue
                        choices_convert.append(''.join(
                            [choices[num_map[n] - 1] for n in choice]))
                    choices = choices_convert
                for _ in range(len(choices), 4):
                    choices.append('无效答案')
                try:
                    label = choices.index(
                        qa['answer']) if 'answer' in qa else None
                    ...
                    logger.error('load data error q: {}, choices: {}'.format(
                        q, choices))
                    pass
        return examples
    ...
    def cut(self, text, q, choices):
        ...
        try:
            max_len = self.data_args.max_seq_length - len(q) - max(
                [len(i) for i in choices if i != '无效答案']) - 4
        except:
    ...
    def compute_metrics(self, eval_predictions):
        ...
        res = {
            'eval_acc': (preds == label_ids).astype(np.float32).mean().item()
        }
        # print('res=', res)
        return res
    ...
    def save_result(self, logits, label_ids):
        ...
        predicts = [{
            'id': k,
            'label': int(v)
        } for k, v in zip([i['eid'] for i in self.test_examples], predictions)]
