...
class PostDataCollatorTagForMultipleChoice:
    ...
    def cut_text(self, text_input):
        ...
        head, tail, a_token = text_input[:a_idx], text_input[
            a_idx + 1:], text_input[a_idx:a_idx + 1]
    ...
    def __call__(self, features):
        ...
        if self.idiom_meaning:
            meaning = [
                self.idiom_meaning[candi] for candi in list(chain(*candidates))
            ]
            meaning_batch = self.tokenizer(meaning,
                                           truncation=True,
                                           max_length=self.meaning_len,
                                           add_special_tokens=False,
                                           return_token_type_ids=False,
                                           return_attention_mask=False)
        flattened_features = []
        ...
        for i in range(batch_size):
            ...
            for j in range(i * num_choices, (i + 1) * num_choices):
                ...
                input_id = [cls_token_id] + head + [self.tag_token_id[0]
                                                    ] + candi_input + [
                                                        self.tag_token_id[1]
                                                    ] + tail + [sep_token_id]
                ...
                # 拼接成语的文字描述
                if self.idiom_meaning:
                    meaning_input = meaning_batch['input_ids'][
                        j]  # 第j个成语的解释文本inputid
                    input_id = input_id + meaning_input + [sep_token_id]
                    token_type_id = token_type_id + [1] * (len(meaning_input) +
                                                           1)
...
class ChidTagProcessor(CLSProcessor):
    def __init__(self, data_args, training_args, model_args):
        ...
        self.train_answer_file = os.path.join(data_args.data_dir,
                                              'train_answer.json')
        self.dev_file = os.path.join(data_args.data_dir, 'dev.json')
        self.dev_answer_file = os.path.join(data_args.data_dir,
                                            'dev_answer.json')
    ...
    def get_examples(self, set_type):
        if set_type == 'train':
            train_answer = self._read_json(
                self.train_answer_file)  # label (正确答案在candidate中的序号)
            examples = self._create_examples(self._read_jsonl(self.train_file),
                                             train_answer, 'train')  # input
            if self.pseudo_train:
                dev_answer = self._read_json(self.dev_answer_file)
                pse1 = self._create_examples(self._read_jsonl(self.dev_file),
                                             dev_answer, 'train')
                # predict_file = '/Users/JianingWang/预训练模型/任务/CLUEMRC/outputs/submit_20220419/chid11_predict.json'
                predict_file = os.path.join(self.data_args.data_dir,
                                            'chid11_predict.json')
                pseudo_answer = self._read_json(predict_file)
                pse2 = self._create_examples(self._read_jsonl(self.test_file),
                                             pseudo_answer,
                                             'train',
                                             pseudo=True)
            ...
            dev_answer = self._read_json(self.dev_answer_file)
            examples = self._create_examples(self._read_jsonl(self.dev_file),
                                             dev_answer, 'dev')
            self.dev_examples = examples
        elif set_type == 'test':
            examples = self._create_examples(self._read_jsonl(self.test_file),
                                             {}, 'test')
    ...
    def set_tokenizer(self, tokenizer):
        ...
        self.anchor_token_id = self.tokenizer.convert_tokens_to_ids(
            self.anchor_token)
        self.tokenizer.add_tokens(self.tag_tokens)
        self.tag_token_ids = self.tokenizer.convert_tokens_to_ids(
            self.tag_tokens)
