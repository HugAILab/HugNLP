...
class CopaProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(guid=guid1,
                             text_a=text_a,
                             text_b=text_b,
                             label=label))
            examples.append(
                InputExample(guid=guid2,
                             text_a=text_a2,
                             text_b=text_b2,
                             label=label2))
        return examples
...
def get_all_data(base_path):
    ...
    for clue_task, path in clue_path.items():
        ...
        train_example = processor.get_train_examples(
            data_dir=os.path.join(base_path, path))
        dev_example = processor.get_dev_examples(
            data_dir=os.path.join(base_path, path))
        test_example = processor.get_test_examples(
            data_dir=os.path.join(base_path, path))
