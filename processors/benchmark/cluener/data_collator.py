...
class DataCollatorForGlobalPointer:
    ...
    def __call__(self, features):
        ...
        if not is_train:
            ...
            labels = torch.zeros(
                len(features), 1, self.max_length,
                self.max_length)  # 阅读理解任务entity种类为1 [bz, 1, max_len, max_len]
            for feature_id, feature in enumerate(features):  # 遍历每个样本
                starts, ends = feature['start'], feature['end']
                offset = feature[
                    'offset_mapping']  # 表示tokenizer生成的token对应原始文本中字符级别的位置区间
                position_map = {}
                # print("offset=", offset)
                    ...
                    if start == 0:
                        ...
                        if start in position_map and end in position_map:
                            ...
                            if self.label_smooth:
                                labels[feature_id, 0, s,
                                       e] = 1 - self.smooth_epsilon
                                labels = self.label_smooth(
                                    labels, s, e, feature_id, self.max_length,
                                    self.smooth_epsilon)
