...
def label_smooth(label_matrix, start, end, feature_id, max_len,
                 smooth_epsilon):
    ...
    for t1, t2 in zip(d1, d2):
        ...
        if start + t1 >= end + t2 and start + t1 >= 0 and end + t2 < max_len:
            if label_matrix[feature_id, 0, start + t1, end + t2] == 0:
                label_matrix[feature_id, 0, start + t1,
                             end + t2] = smooth_epsilon / dis
            if label_matrix[feature_id, 0, start + t1,
                            end + t2] == smooth_epsilon / 2 and dis == 1:
                label_matrix[feature_id, 0, start + t1,
                             end + t2] = smooth_epsilon
    return label_matrix
...
class CLUENERProcessor(CLSProcessor):
    ...
    def get_examples(self, set_type):
        if set_type == 'train':
            examples = self._create_examples(self._read_json2(self.train_file),
                                             'train')
            ...
            examples = self._create_examples(self._read_json2(self.dev_file),
                                             'dev')
            examples = examples[:self.data_args.max_eval_samples]
            self.dev_examples = examples
        elif set_type == 'test':
            examples = self._create_examples(self._read_json2(self.test_file),
                                             'test')
    ...
    def _create_examples(self, lines, set_type):
        ...
        for line in lines:
            ...
            for label, label_word in self.verbalizer.items():
                ...
                start, end = list(), list()
                prompt_text = self.prompt[self.prompt_id].format(
                    desc, label_word, text)
                new_len = len(
                    self.prompt[self.prompt_id]) - 7 + len(desc) + len(
                        label_word)  # 新增的token数量
    ...
    def get_predict_result(self, logits, examples):
        ...
        for prob, index, example in zip(probs, indices, examples):
            ...
            new_len = len(self.prompt[self.prompt_id]) - 7 + len(
                self.label_desc[example['type']]) + len(
                    self.verbalizer[example['type']])
            ...
            for ei, entity in enumerate(entity_index):
                ...
                start_end = np.unravel_index(entity,
                ...
                if s == -1 and e == -1:
                    ...
                    if len(ans) > 1:
                        if _is_chinese_char(
                                ans[0]) and not _is_chinese_char(ans[1]):
                            ans = ans[1:]
                            s = s + 1
                    ...
                    # 吉普森阶梯g
                    if len(ans) > 1:
                        if ans[-2] == '》' or (_is_chinese_char(ans[-2])
                                              and _is_eng_or_pun(ans[-1])):
                            ans = ans[:-1]
                            e = e - 1
    ...
    def compute_metrics(self, eval_predictions):
        ...
        golden, preds, dataname_map, dataname_type = {}, {}, defaultdict(
            list), {}
        ...
        for example in examples:
            ...
            preds_ = [
                i['answer'] for i in predictions[id_][label]
                if i['answer'] != ''
            ]
            if len(preds_) == 0:
                preds_ = ['']
            if len(golden_) == 1 and len(
                    preds_) == 1 and golden_[0] == '' and preds_[0] == '':
                continue
    ...
    def save_result(self, logits, label_ids, stage='test'):
        def search_all_span(entity, text):
            ...
            if entity == '':
                return list()
            spans = [[i, i + len(entity) - 1] for i in range(len(text))
                ...
                for span in spans:
                    ...
                    for other_span in span_list:
                        if span == other_span:  # 排除自己的区间
                            continue
                        if span[0] >= other_span[0] and span[1] <= other_span[
                                1]:  # 存在嵌套，当前的实体span被其他span包含
                            yes = True
                            break
        ...
        examples = self.raw_datasets[
            'test'] if stage == 'test' else self.raw_datasets['validation']
        ...
        for k, v in predicts.items():
            ...
            for type, answer in v.items():
                ...
                for ans in answer:
                    ...
                    # span = ans['pos']
                    answer_num += 1
                    type_res[entity] = search_all_span(
                        entity, examples[exampleid2index[k]]['text'])
                if answer_num > 0:
                    res[type] = type_res
        ...
        output_submit_file = os.path.join(
            self.training_args.output_dir, 'cluener_predict.json'
            if stage == 'test' else 'cluener_dev_predict.json')
        # 保存标签结果
