...
class CLUEProcessor(CLSProcessor):
    ...
    def get_examples(self, set_type):
        def read_pseudo(input_file):
            ...
            with open(input_file, 'r', encoding='utf-8') as f:
                ...
                for line in reader:
                    ...
                    if float(pseudo_data['pseudo_proba']
        ...
        if set_type == 'train':
            if self.is_pseudo:
                train_lines = self._read_json2(
                    os.path.join(self.data_dir, 'train.json'))
                dev_pseudo_num, test_pseudo_num = 0, 0
                if os.path.exists(
                        os.path.join(self.data_dir, 'dev_pseudo.json')):
                    dev_pseudo_lines = read_pseudo(
                        os.path.join(self.data_dir, 'dev_pseudo.json'))
                    train_lines.extend(dev_pseudo_lines)
                    dev_pseudo_num += len(dev_pseudo_lines)
                if os.path.exists(
                        os.path.join(self.data_dir, 'test_pseudo.json')):
                    test_pseudo_lines = read_pseudo(
                        os.path.join(self.data_dir, 'test_pseudo.json'))
                ...
                print('add pseudo all num={}'.format(
                    str(dev_pseudo_num + test_pseudo_num)))
                self.train_examples = examples
            else:
                examples = self._create_examples(
                    self._read_json2(self.train_file), 'train')
    ...
    def get_predict_result(self, logits, examples, stage='dev'):
        ...
        # 保存标签结果
        with open(os.path.join(self.data_dir, '{}_pseudo.json'.format(stage)),
                  'w') as writer:
...
class CSLEFLProcessor(CLUEProcessor):
    def get_predict_result(self, logits, examples, stage='dev'):
        ...
        if stage == 'train':
            for pred, example, logit in zip(preds, examples, logits):
                id_ = example[
                    'guid']  # 验证集和训练集一样，每个abst文本与一个关键词组成example，所以这里的guid并不是原始数据集的编号
                id_ = int(id_.split('-')[1])
                predictions[id_] = pred  # 保存预测结果索引labelid
                # 获取TopK结果
                # {'prob': prob, 'answer': answer}
            ...
            for id_, probas in pred_probas.items():
                ...
                for key in keys:
                    if len(key) == 2:
                        key_length_num += 1
                print('id={}, probas={}'.format(
                    id_, ' '.join([str(prob) for prob in probas])))
...
class TnewsEFLProcessor(CLSProcessor):
    ...
    def _create_examples(self, lines, set_type, label_desc):
        examples = self.processor.create_examples(lines,
                                                  set_type,
    ...
    def compute_metrics(self, eval_predictions):
        ...
        golden, dataname_map, dataname_type = {}, defaultdict(list), {}
        predictions, _ = self.get_predict_result(
            eval_predictions[0], examples)  # {'xx': 'xxx', ...}
        for example in examples:
            data_type = self.output_modes
            data_name = self.data_name
            if data_name not in dataname_type:
        ...
        all_metrics['macro_acc'] = round(
            all_metrics['acc'] / len(dataname_map), 4)
