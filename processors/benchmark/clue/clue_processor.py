    ...
    def create_examples(self,
        ...
        for (i, line) in enumerate(lines):
            ...
            for label_class, desc_text in label_desc.items():
                ...
                if use_keys:
                    text_a = '{}。{}。{}'.format(
                        '这是一篇关于【{}】的新闻'.format(desc_text), keywords,
                        line['sentence'])
                else:
                    text_a = '{}。{}'.format('这是一篇关于【{}】的新闻'.format(desc_text),
                                            line['sentence'])
                examples.append({
                    'guid': guid,
                    'text_a': text_a,
    ...
    def create_examples(self, lines, set_type):
        ...
        if set_type == 'train':
            ...
            for (i, line) in enumerate(lines):
                ...
                # label = str(line['label']) if set_type != 'test' else '0'
                if text_b not in abst2keys.keys(
                ):  # 如果不存在，则抛弃该文本，因为我们无法知道标签为0的哪些关键词是对的
                    continue
                correct_keys = abst2keys[text_b]
                label = str(line['label'])
                if label == '0':
                    for k in keywords:
                        if k not in correct_keys.keys(
                        ):  # 只有标签为0，且当前的关键词不在已知的正确标签集合里，这些标签是不能表示当前文章的
            ...
            for abst, key2label in abst2keys.items():
                ...
                for key, label in key2label.items():
                    examples.append({
                        ...
                        guid,
                        'text_a':
                        '【{}】可以作为文章的关键词吗？文章：【{}】'.format(key, abst),
                        'text_b':
                        None,
                        'label':
                        label_map[label],
...
class CopaProcessor(DataProcessor):
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = str(1 if line['label'] ==
                        0 else 0) if set_type != 'test' else '0'
            choice1 = line['choice1']
            label2 = str(0 if line['label'] ==
                         0 else 1) if set_type != 'test' else '0'
