...
class TextClassificationProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self.create_examples(
            self._read_json(os.path.join(data_dir, 'train_few_all.json')),
            'train')
    ...
    def get_labels(self):
        ...
        if self.task_name == 'mr':
            ...
            return [
                100, 101, 102, 103, 104, 106, 107, 108, 109, 110, 112, 113,
                114, 115, 116
            ]
        elif self.task_name == 'csldcp':
            return [
                '材料科学与工程', '作物学', '口腔医学', '药学', '教育学', '水利工程', '理论经济学',
                '食品科学与工程', '畜牧学/兽医学', '体育学', '核科学与技术', '力学', '园艺学', '水产', '法学',
                '地质学/地质资源与地质工程', '石油与天然气工程', '农林经济管理', '信息与通信工程',
                '图书馆、情报与档案管理', '政治学', '电气工程', '海洋科学', '民族学', '航空宇航科学与技术',
                '化学/化学工程与技术', '哲学', '公共卫生与预防医学', '艺术学', '农业工程', '船舶与海洋工程',
                '计算机科学与技术', '冶金工程', '交通运输工程', '动力工程及工程热物理', '纺织科学与工程', '建筑学',
                '环境科学与工程', '公共管理', '数学', '物理学', '林学/林业工程', '心理学', '历史学',
                '工商管理', '应用经济学', '中医学/中药学', '天文学', '机械工程', '土木工程', '光学工程',
                '地理学', '农业资源利用', '生物学/生物科学与工程', '兵器科学与技术', '矿业工程', '大气科学',
                '基础医学/临床医学', '电子科学与技术', '测绘科学与技术', '控制科学与工程', '军事学', '中国语言文学',
                '新闻传播学', '社会学', '地球物理学', '植物保护'
            ]
        ...
        for (i, line) in enumerate(lines):
            ...
            if self.task_name in ['eprstmt']:
                ...
                examples.append({
                    ...
                    None,
                    'label':
                    label_map[line['label']] if set_type != 'test' else '',
                    'is_train':
                    1 if set_type != 'test' else 0
                })
...
class OcnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        train_lines = self._read_json(
            os.path.join(data_dir, 'train_few_all.json'))
        if os.path.exists(os.path.join(data_dir, 'dev_pseudo.json')):
            dev_pseudo_lines = self._read_json(
                os.path.join(data_dir, 'dev_pseudo.json'))
            train_lines.extend(dev_pseudo_lines)
        if os.path.exists(os.path.join(data_dir, 'test_pseudo.json')):
            test_pseudo_lines = self._read_json(
                os.path.join(data_dir, 'test_pseudo.json'))
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if pronoun_idx > query_idx:
                ...
                s1, e1, s2, e2 = query_idx, query_idx + len(
                    query) + 1, pronoun_idx + 2, pronoun_idx + len(
                        pronoun) + 2 + 1
            else:
                ...
                s1, e1, s2, e2 = pronoun_idx, pronoun_idx + len(
                    pronoun) + 1, query_idx + 2, query_idx + len(query) + 2 + 1
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append({
                ...
                text_b,
                'label':
                label_map[label] if 'label' in line.keys() else '',
                'is_train':
                1 if set_type != 'test' else 0
            })
        return examples
