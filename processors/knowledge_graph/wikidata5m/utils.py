"""用于读取wikidata知识图谱 download wikidata5m: https://deepgraphlearning.github.io/project/wikidata5m."""
...
class Wikidata5m:
    ...
    def load_entity(self):
        """Q5196650	Cut Your Hair	cut your hair Q912600	Straumur-Burðarás	Straumur	straumur–burðarás Q47551	ditiano	tipciano	titiaen geovene return Union(Dict[str, List], Dict[str, List])"""
        print('loading wikidata entity')
        entity_qid2name = dict()
        name2entity_qid = dict()
        with open(os.path.join(self.data_dir, 'wikidata5m_entity.txt'),
                  'r',
                  encoding='utf-8') as fr:
            lines = fr.readlines()
        for line in tqdm(lines):
            line = line.replace('\n', '')
            entity_qid, entity_name_list = line.split('\t')[0], line.split(
                '\t')[1:]
            # qid -> name
    ...
    def load_relation(self):
        """P489	currency symbol description P834	train depot	railway depot	depot	rail yard P2629	BBFC rating	BBFC certificate P1677	index case of return Dict[str, Dict[str, List]]"""
        print('loading wikidata relation')
        relation_pid2name = dict()
        name2relation_pid = dict()
        with open(os.path.join(self.data_dir, 'wikidata5m_relation.txt'),
                  'r',
                  encoding='utf-8') as fr:
            lines = fr.readlines()
        for line in tqdm(lines):
            line = line.replace('\n', '')
            relation_pid, relation_name_list = line.split('\t')[0], line.split(
                '\t')[1:]
            # qid -> name
    ...
    def load_relation_template(self):
        if not os.path.exists(os.path.join(self.data_dir, 'relations.jsonl')):
            return {}
        with open(os.path.join(self.data_dir, 'relations.jsonl'),
                  'r',
                  encoding='utf-8') as fr:
            lines = fr.readlines()
    ...
    def load_triple(self):
        """Q29387131	P31	Q5 Q326660	P1412	Q652 Q7339549	P57	Q1365729 Q554335	P27	Q29999 Q20641639	P54	Q80955 return Dict[str, Dict[str, List]]"""
        print('loading wikidata triplet')
        entity_subgraph = dict()
        with open(os.path.join(self.data_dir, 'wikidata5m_all_triplet.txt'),
                  'r',
                  encoding='utf-8') as fr:
            lines = fr.readlines()
        for line in tqdm(lines):
    ...
    def search_triple(self, head_qid, tail_qid, k_hop=1):
        ...
        for relation_pid in sub_graph.keys():
            ...
            if tail_qid in tail_entities:
                return (head_entity_name, relation_name,
                        tail_entity_name), (head_qid, relation_pid, tail_qid)
        return None
    ...
    def tokenize_template(self):
        ...
        return ' '.join(map(str, trigger_x)), ' '.join(map(
            str, trigger_y)), relation_pid2template_ids_str
    ...
    def tagme_annotation(self, txt):
        def Annotation_mentions(txt):
            ...
            for mention in annotation_mentions.mentions:
                try:
                    dic[str(mention).split(' [')[0]] = str(mention).split(
                        '] lp=')[1]
                except:
