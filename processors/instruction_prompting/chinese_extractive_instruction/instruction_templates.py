...
def instruction_format(data_info: Dict, data_type, data_name) -> List[Dict]:
    ...
    for instruction in [dataset2instruction]:
        ...
        instruction_processor = format_info['instruction'](
            data_name, data_list, label_mappings, format_info['prompt'],
...
if __name__ == '__main__':
    ...
    type2data = {
        'similarity': [
            'afqmc.json', 'bq_corpus.json', 'chip-cdn.json', 'lcqmc.json',
            'oppo.json', 'paws-x-zh.json', 'sohu_sts_a_ll.json',
            'sohu_sts_a_sl.json', 'sohu_sts_a_ss.json', 'sohu_sts_b_ll.json',
            'sohu_sts_b_sl.json', 'sohu_sts_b_ss.json'
        ],
        'news': [
            'baidu_news.json', 'ccfbdci2020.json', 'chinanews.json',
            'ifeng.json', 'thucnews.json'
        ],
        'multichoice': ['c3.json'],
        'mrc': [
            'cail.json', 'ccm_qa.json', 'cmrc2018.json', 'drcd.json',
            'dureader_checklist.json', 'dureader_robust.json', 'squad.json'
        ],
        'yesno': ['cail_yesno.json'],
        'sentiment': [
            'car_emotion.json', 'chnsenticorp.json', 'covid_emotion.json',
            'jd_binary_waimai_10k.json', 'jd_full.json', 'nlpcc14-sc.json',
            'online_shopping_10_cats.json', 'simplifyweibo_4_moods.json',
            'turing_emotion.json', 'weibo_senti_100k.json', 'yf_amazon.json',
            'yf_dianping.json'
        ],
    ...
    for data_type, files in tqdm(type2data.items()):
        for file in files:
            ...
            data = json.load(
                open(os.path.join(base_path, file), encoding='utf8'))
    ...
    with open(os.path.join(base_path, 'train_mrc_instruction.json'),
              'w',
              encoding='utf8') as fout:
        json.dump(train_data, fout, indent=2, ensure_ascii=False)
    with open(os.path.join(base_path, 'dev_mrc_instruction.json'),
              'w',
              encoding='utf8') as fout:
        json.dump(dev_data, fout, indent=2, ensure_ascii=False)
    with open(os.path.join(base_path, 'test_mrc_instruction.json'),
              'w',
              encoding='utf8') as fout:
