...
predict_result_files = [
    ...
    'chinese_pretrain_mrc_roberta_wwm_ext_large', 'chinese-pert-large',
    'chinese-pert-large-mrc', 'structbert-large-zh'
]
...
def merge_output(predict_result_files):
    ...
    best = {
        k: v.most_common()[0][0]
        for k, v in out.items() if 'ner' not in k.lower()
    }
    for k, v in out.items():
        if 'ner' in k.lower():
            best[k] = [
                m for m, n in v.items() if n > 0.6 * len(predict_result_files)
            ]
        else:
...
def obtain_confused_result(out: defaultdict,
                           threshole=0.6,
                           confuse_value=0.15,
                           num_template=2):
    ...
    for k, v in out.items():
        ...
        second_result, second_prob = v.most_common()[1][0], v.most_common(
        )[1][1]
        if best_prob <= threshole * len(predict_result_files) * num_template:
            is_filter = True
        if abs(best_prob - second_prob
               ) < confuse_value * len(predict_result_files) * num_template:
            is_filter = True
        if is_filter:
            num = len(v.most_common())
            hard_answer[k] = {
                v.most_common()[i][0]: v.most_common()[i][1]
                for i in range(num)
            }
