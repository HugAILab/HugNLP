...
class BertForPretrainWithKG(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifiers = nn.ModuleList([
            nn.Linear(config.hidden_size, config.num_ner_labels)
            for _ in range(config.entity_type_num)
        ])
        self.post_init()
    ...
    def forward(
        ...
        if ner_labels is not None:
            ...
            active_loss = attention_mask.repeat(self.config.entity_type_num, 1,
                                                1).view(-1) == 1
            active_logits = ner_logits.reshape(-1, self.config.num_ner_labels)
            active_labels = torch.where(
                active_loss, ner_labels.view(-1),
...
class BertForPretrainWithKGV2(BertPreTrainedModel):
    ...
    def simcse_unsup_loss(y_pred: 'tensor') -> 'tensor':
        ...
        sim = F.cosine_similarity(y_pred.unsqueeze(1),
                                  y_pred.unsqueeze(0),
                                  dim=-1)
