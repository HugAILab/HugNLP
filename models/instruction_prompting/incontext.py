...
class GPT2ForInContextClassification(GPT2LMHeadModel):
    def forward(
        ...
        label_masks: Optional[
            torch.
            LongTensor] = None,  # mask=1 means it should be calculated loss
        options: Optional[list] = None,  # 如果是分类任务，则可以添加候选label
        output_attentions=None,
        output_hidden_states=None,
        return_dict=None,
    ):
        assert len(input_ids.shape) == 3 and input_ids.shape[1] == len(
            options)  # [n, option_size, len]
        batch_size = input_ids.shape[0]
        option_size = input_ids.shape[1]
        input_ids = input_ids.view(-1, input_ids.shape[1],
                                   input_ids.shape[2])  # [n*option_size, len]
        attention_mask = attention_mask.view(
            -1, input_ids.shape[1], input_ids.shape[2]
        ) if attention_mask is not None else None  # [n*option_size, len]
        token_type_ids = token_type_ids.view(
            -1, input_ids.shape[1], input_ids.shape[2]
        ) if token_type_ids is not None else None  # [n*option_size, len]
        ...
        hidden_states = transformer_outputs[
            0]  # [n*option_size, len, hidden_size]
        lm_logits = self.lm_head(
            hidden_states)  # [n*option_size, len, vocab_size]
        lm_logits = lm_logits.view(batch_size, option_size,
                                   input_ids.shape[-1],
                                   -1)  # [n, option_size, len, vocab_size]
        ...
        if labels is not None:
            for label, lm_logit in zip(labels, lm_logits):
                ...
                print('shift_labels.shape=', shift_labels.shape)
                loss = [
                    loss_fct(shift_logit.view(-1, shift_logit.size(-1)),
                             shift_label.view(-1))
                    for shift_logit, shift_label in zip(
                ...
                if label_masks is not None:
                    loss = loss.view(
                        lm_logits.size(0),
                        lm_logits.size(1)) * label_masks  # [option_size, len]
                    loss = torch.sum(loss, axis=1) / torch.sum(
                        label_mask, axis=1)  # [option_size]
                losses.append(loss)
...
if __name__ == '__main__':
    ...
    model = GPT2ForInContextClassification.from_pretrained(
        '/Users/wangjianing/Desktop/开源代码与数据模型/模型/gpt2')
    ...
    inputs['input_ids'] = inputs['input_ids'].view(
        -1, inputs['input_ids'].shape[0], inputs['input_ids'].shape[1])
