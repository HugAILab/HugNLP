...
def split_context_query(sequence_output, pq_end_pos, input_ids):
    ...
    context_sequence_output = sequence_output.new(
        torch.Size((sequence_output.size(0), context_max_len,
                    sequence_output.size(2)))).zero_()
    query_sequence_output = sequence_output.new_zeros(
        (sequence_output.size(0), query_max_len, sequence_output.size(2)))
    query_attention_mask = sequence_output.new_zeros(
        (sequence_output.size(0), query_max_len))
    context_attention_mask = sequence_output.new_zeros(
        (sequence_output.size(0), context_max_len))
    for i in range(0, sequence_output.size(0)):
        p_end = pq_end_pos[i][0]
        q_end = pq_end_pos[i][1]
        context_sequence_output[
            i, :min(context_max_len, p_end)] = sequence_output[
                i, 1:1 + min(context_max_len, p_end)]
        idx = min(query_max_len, q_end - p_end - sep_tok_len)
        query_sequence_output[i, :idx] = sequence_output[
            i, p_end + sep_tok_len + 1:p_end + sep_tok_len + 1 +
            min(q_end - p_end - sep_tok_len, query_max_len)]
        query_attention_mask[i, :idx] = sequence_output.new_ones(
            (1, query_max_len))[0, :idx]
        context_attention_mask[
            i, :min(context_max_len, p_end)] = sequence_output.new_ones(
                (1, context_max_len))[0, :min(context_max_len, p_end)]
...
class BertDUMAForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        flat_head_mask = head_mask.view(
            -1, head_mask.size(-1)) if head_mask is not None else None
        flat_inputs_embeds = inputs_embeds.view(
            -1, inputs_embeds.size(-1)) if inputs_embeds is not None else None
        ...
        for _ in range(0, iter):
            cq_biatt_output = self.bert_att(context_sequence_output,
                                            query_sequence_output,
                                            context_attention_mask)
            qc_biatt_output = self.bert_att(query_sequence_output,
                                            context_sequence_output,
        ...
        cat_output = torch.cat(
            [torch.mean(qc_biatt_output, 1),
             torch.mean(cq_biatt_output, 1)], 1)
...
class RobertaDUMAForMultipleChoice(RobertaPreTrainedModel):
    ...
    def forward(self,
        ...
        outputs = self.roberta(input_ids=flat_input_ids,
                               attention_mask=flat_attention_mask,
                               token_type_ids=flat_token_type_ids,
                               position_ids=flat_position_ids,
                               head_mask=flat_head_mask,
