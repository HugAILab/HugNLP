...
def print_hello():
    ...
    print('+' + '-' * (length - 2) + '+')
    print('|' + ' ' * (length - 2) + '|')
    print(' ' + ' ' * int((length - 2 - 25) / 2) + '🤗 Welcome to use HugNLP!' +
          ' ' * int((length - 2 - 25) / 2) + ' ')
    print('' + ' ' * (length) + '')
    print('' + ' ' * int((length - 2 - 32) / 2) +
...
def main():
    ...
    training_args.output_dir = os.path.join(
        ...
        list(filter(None, model_args.model_name_or_path.split('/')))[-1])
    ...
    if os.path.isdir(
        ...
        if last_checkpoint is None and len([
                i for i in os.listdir(training_args.output_dir)
    ...
    tokenizer_class = TOKENIZER_CLASSES.get(model_args.model_type,
                                            AutoTokenizer)
    ...
    if data_args.task_name in PROCESSORS:
        processor = PROCESSORS[data_args.task_name](data_args,
                                                    training_args,
                                                    model_args,
                                                    tokenizer=tokenizer)
    else:
    ...
    if model_args.use_prompt_for_cls:
        assert hasattr(
            processor, 'label_word_list'
        ), 'If you use prompt, you must design label_word_list in processor.'
    ...
    if model_args.freeze_epochs:
        callbacks.append(
            FreezeCallback(freeze_epochs=model_args.freeze_epochs,
                           freeze_keyword=model_args.freeze_keyword))

    if model_args.ema:
        callbacks.append(
            ExponentialMovingAveragingCallback(model_args.ema_decay))
